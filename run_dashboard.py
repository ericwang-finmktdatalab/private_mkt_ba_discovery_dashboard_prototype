...
sector_group = df.groupby("SECTOR").apply(
    lambda x: (x["EBITDA_GROWTH"] * x["MARKET_VALUE"]).sum() / x["MARKET_VALUE"].sum()
).reset_index(name="SECTOR_EBITDA_GROWTH")
...
fund_group = df.groupby("PARENT_FUND").apply(
    lambda x: (x["EBITDA_GROWTH"] * x["MARKET_VALUE"]).sum() / x["MARKET_VALUE"].sum()
).reset_index(name="FUND_EBITDA_GROWTH")
